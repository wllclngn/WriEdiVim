...
let s:_lightline = {
      \   'active': {
      \     'left': [['mode'], ['filename'], ['filetype'], ['filencoding'], ['updated']],
      \     'right': [['wordcount'], ['bytecount'], ['lines'], ['column']]
      \   },
      \   'inactive': {
      \     'left': [['filename']],
      ...
      \   'component': {
      \     'mode': ' %{lightline#mode()} ',
      \     'filename': '  %t', 'filetype': ' %Y', 'filencoding': ' %{Encoding()}', 'updated':  ' %{Modified()}', 'column': ' C%c', 'lines':' L%l/%L', 'bytecount': ' %{Biter()}', 'wordcount': ' %{wordcount().words . "W"}  ', 'percent':'%P' 
      \   },
      ...
      \   'colorscheme': 'darcula',
      \   'mode_map': {
      \     'n': ' NORMAL ', 'i': ' INSERT ', 'R': ' REPLACE ', 'v': ' VISUAL ', 'V': ' VILINE ', "\<C-v>": ' VBLOCK ',
...
func! Encoding()
  if (&fileencoding?&fileencoding:&encoding == "utf-8")
    return "UTF-8"
  elseif (&fileencoding?&fileencoding:&encoding == "utf-8")
    return "UTF-8LE"
  elseif (&fileencoding?&fileencoding:&encoding == "utf-16")
    return "UTF-16"
  elseif (&fileencoding?&fileencoding:&encoding == "utf-16le")
    return "UTF-16LE"
  elseif (&fileencoding?&fileencoding:&encoding == "utf-32")
    return "UTF-32"
  elseif (&fileencoding?&fileencoding:&encoding == "utf-32le")
    return "UTF-32LE"
  elseif (&fileencoding?&fileencoding:&encoding == "ucs-2")
    return "UCS-2"
  elseif (&fileencoding?&fileencoding:&encoding == "ucs-2le")
    return "UCS-2LE"
  elseif (&fileencoding?&fileencoding:&encoding == "ucs-4")
    return "UCS-4"
  elseif (&fileencoding?&fileencoding:&encoding == "ucs-4le")
    return "UCS-4LE"
  elseif (&fileencoding?&fileencoding:&encoding == "latin1")
    return "ISO 8859-1"
  elseif (&fileencoding?&fileencoding:&encoding == "macroman")
    return "MacRoman [Macintosh]"
  else
...
func! Biter()
  if (wordcount().bytes < 1024)
    return wordcount().bytes . "B"
  elseif (wordcount().bytes >= 1048576)
    return (wordcount().bytes / 1048576) . "MB"
  else
    return (wordcount().bytes / 1024) . "KB"
  endif
endfu
...
function! lightline#colorscheme() abort
  try
    ...
    let s:lightline.colorscheme = 'Tomorrow'
    let s:lightline.palette = g:lightline#colorscheme#{s:lightline.colorscheme}#palette
  finally
